...
namespace hud {
    void set_player(player * a_player)
    {
        pl = a_player;
    ...
    void show_lives() {
        ...
        room*& this_room = game_actor_generic::current_room;

        Texture target_txt(gfx::_get_render(), game_actor_generic::current_room->win_width(),
                                                game_actor_generic::current_room->win_height(),
    ...
    void check_status()
    {
        ...
        if(!(pl->is_death || pl->current_room->bgm.is_playing()))
            game_actor_generic::current_room->bgm.play();
                ...
                game_actor_generic::current_room->win_height())
        {
            ...
            // In tal caso, assicurarsi di riavviare il livello solo dopo che il giocatore cada per l'ennesima volta.
                ...
                game_actor_generic::current_room->restart();
        }
        ...
        if(pl->is_death && !managing_death) {
            ...
            pl->set_death();
            life_counter--;
            game_actor_generic::current_room->sleep(400);
            managing_death = true;
        }
    }
}
